...
import numpy as np
import decision_transformer.dataset.pytorch_util as ptu
...
MODEL_PATH = {
    "IQL": "/apdcephfs/share_1563664/ztjiaweixu/rdt_sz/2024042000",
...
class Evaluation_Attacker:
    ...
    def load_model(self):
        ...
        for root, dirs, files in os.walk(model_path):
            if f"{self.agent_name}_{self.env_name}_0_" in root:
        ...
        if self.agent_name == "IQL":
            ...
            # self.q_fun = (
            #     TwinQ(self.obs_dim, self.action_dim, n_hidden=2)
            #     .to(ptu.device)
            #     .eval()
            # )
            self.policy.load_state_dict(state_dict["actor"])
            # self.q_fun.load_state_dict(state_dict["qf"])
        else:
            raise NotImplementedError
...
class Attack:
    ...
    def load_model(self):
        ...
        if self.agent_name == "IQL":
            from decision_transformer.dataset.IQL import DeterministicPolicy, TwinQ
